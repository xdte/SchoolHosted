...
while True:
    for direction in stops:
        formatted = {}
        if direction == "MTR":
            response = requests.get("https://rt.data.gov.hk/v1/transport/mtr/getSchedule.php?line=KTL&sta=CHH").json()["data"]["KTL-CHH"]
            formatted["MTR to Tiu Keng Leng"] = {}
            formatted["MTR to Tiu Keng Leng"]["ETA"] = []
            formatted["MTR to Whampoa/Ho Man Tin"] = {}
            formatted["MTR to Whampoa/Ho Man Tin"]["ETA"] = []
            for i in response["UP"]:
                formatted["MTR to Tiu Keng Leng"]["ETA"].append(i["ttnt"])
            formatted["MTR to Tiu Keng Leng"]["ETA"] = " | ".join(formatted["MTR to Tiu Keng Leng"]["ETA"])
            for i in response["DOWN"]:
                formatted["MTR to Whampoa/Ho Man Tin"]["ETA"].append(i["ttnt"])
            formatted["MTR to Whampoa/Ho Man Tin"]["ETA"] = " | ".join(formatted["MTR to Whampoa/Ho Man Tin"]["ETA"])
        else:
        ...
        with placeholder.container():
            ...
            if direction != "MTR":
                ...
                st.write('Choi Hung MTR')
            st.table(df)
            st.write(f"Last Updated: {str(datetime.now(pytz.timezone('Hongkong')))[:-13]}")
            st.caption("v1.2.5 Made with ❤ by Bonfire")
        time.sleep(6 if direction!="MTR" else 4)
